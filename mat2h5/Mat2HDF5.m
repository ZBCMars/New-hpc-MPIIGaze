...
filepath = 'D:\CHUANGXIN\dataset\MPIIGaze\Data\Normalized\p14';
 
% filepath = '/disks/disk0/fjl/MPIIGaze/Data/Original';
...
for num_f=1:length(files)
    %readname = [files{num_f}];
    readname = [ 'D:\CHUANGXIN\dataset\MPIIGaze\Data\Normalized\p14','/',files{num_f}];
    temp = load(readname);
...
%for num_f=1:length(files)
    %folderpath = [filepath, '/', files{num_f}];
    %path2 = dir(folderpath);
    %filefolder = {path2.name};
    %for num_'
...
Data.data = zeros(36,60,1, total_num*2);
...
for num_f=1:length(files)
    ...
    for num_i=1:num_data
        ...
        index = index+1;
        img = zeros(36,60);
        Data.data(:, :, 1, index) = double(flip(img, 2));
