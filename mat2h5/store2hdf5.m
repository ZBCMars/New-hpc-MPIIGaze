function [curr_dat_sz, curr_lab_sz] = store2hdf5(filename, data, labels, create, chunksz, deflate, data_name, label_name, startloc)  
  ...
  if create
    ...
    h5create(filename, ['/',data_name], [dat_dims(1:end-1) Inf], 'Datatype', 'single', 'ChunkSize', [dat_dims(1:end-1) chunksz], 'Deflate', deflate); % width, height, channels, number 
    h5create(filename, ['/',label_name], [lab_dims(1:end-1) Inf], 'Datatype', 'single', 'ChunkSize', [lab_dims(1:end-1) chunksz], 'Deflate', deflate); % width, height, channels, number
  ...
  if ~isempty(data)
    h5write(filename, ['/',data_name], single(data), startloc.dat, size(data));
    h5write(filename, ['/',label_name], single(labels), startloc.lab, size(labels));  
  end
